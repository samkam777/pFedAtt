...
class PerSampleGradientClipper:
    ...
    def __init__(
        ...
        self.norm_clipper = norm_clipper        # norm_clipper的类？ 就是util文件夹下的clipping.py里的ConstantFlatClipper
    ...
    def clip_and_accumulate(self) -> None:
        ...
        all_norms = calc_sample_norms(
            named_params=self._named_grad_samples(),    # Helper function to get names and per-sample gradients for parameters that required grad.
            flat=not self.norm_clipper.is_per_layer,
        )

        # step 1: calculate the clipping factors based on the noise
        clipping_factor = self.norm_clipper.calc_clipping_factors(all_norms)    # 计算clipping factor，使其最多等于max norm
        ...
        for i, (clip_factor, named_param) in enumerate(
            ...
            self._on_batch_clip(
                ...
                grad_after_clip=self._scale_summed_grad(summed_grad, batch_size),   # 这是加了噪声再clip的
            )
    ...
    def _on_batch_clip(
        ...
        # 打印被裁切的，看看有什么变化
        #print("grad before clip:{}\ngrad after clip:{}\n".format(grad_before_clip, grad_after_clip))   # 为什么前后的grad每个都有变化，不应该只截取超过部分吗？
