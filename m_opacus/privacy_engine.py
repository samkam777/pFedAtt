...
class PrivacyEngine:
    ...
    def attach(self, optimizer: torch.optim.Optimizer):
        ...
        def dp_step(self, closure=None):
            self.privacy_engine.step()                          # 指的是下面的那个optimizer.privacy_engine
            self.original_step(closure)

        optimizer.privacy_engine = self
        optimizer.original_step = optimizer.step                # 原来优化器的step
        optimizer.step = types.MethodType(dp_step, optimizer)   # 使用MethodType()方法将函数绑定到实例或者是类上
    ...
    def step(self):
        ...
        for p in self.module.parameters():
            p_orignal_grad_list.append(p.grad)
        #print("orignal gradient:{}\n".format(p_orignal_grad_list[10]))

        self.steps += 1
        self.clipper.clip_and_accumulate()
        clip_values, batch_size = self.clipper.pre_step()

        #print("clip_values:{}\n".format(clip_values))   # 就等于我们设置的max norm
        ...
        for p, clip_value in zip(params, clip_values):
            ...
            if self.loss_reduction == "mean":
                noise /= batch_size
            #print("orignal grad:{}\nnoise:{}\n".format(p.grad, noise))
            p_clip_grad_list.append(p.grad)
            p.grad += noise
            p_noise_grad_list.append(p.grad)
            #print("add noise grad:{}\n".format(p.grad))
        #print("clip gradient:{}\nnoise gradient:{}\n".format(p_clip_grad_list[10], p_noise_grad_list[10]))
